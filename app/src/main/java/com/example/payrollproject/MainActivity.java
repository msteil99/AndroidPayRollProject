...
*
 ...
 //dead code lies here
/*
    private boolean openSettingsFrag() {


     settingsFragment = new SettingsFragment();
     fragTrans = fragMan.beginTransaction();
     fragTrans.remove(nextPayFragment);
     fragTrans.add(R.id.flFragment,settingsFragment).commit();
     return isSettingsFrag = true;
    }
    /*
    private boolean openNextPayFrag() {
       nextPayFragment = new NextPayFragment();
       fragTrans = fragMan.beginTransaction();
       fragTrans.add(R.id.flFragment2, nextPayFragment).commit();
       return isNextPayFrag = true;
    }
*/
/*
    //could combine these two methods in to one if btn pressed =  setting else
    private boolean openCurDateFrag(){
      fragTrans.remove(nextPayFragment);
     //pass the data from Activity to Fragment via key/value pair in a bundle
     Bundle bundle = new Bundle();
     bundle.putString("dateKey",dateKey);
     //instantiate the fragment object and pass the bundle
     dateFragment = new CurrentDateFragment();
     dateFragment.setArguments(bundle);
     //now add the Fragment to the frameLayout in main activity
     //fragMan= getSupportFragmentManager();
     fragTrans = fragMan.beginTransaction();
     fragTrans.add(R.id.flFragment,dateFragment).commit();
     return isDateFrag = true;
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
     ...
     //try to create a list of fragment
     ...
     settingsView = findViewById(R.id.imSetting);
     settingsView.setOnClickListener(new View.OnClickListener() {
      @Override
      public void onClick(View view) {
     //  openSettingsFrag();
       //lets move the fragments into here with a nicer interface
          settingsFragment = new SettingsFragment();
          fragTrans = fragMan.beginTransaction();
          fragTrans.replace(R.id.flFragment,settingsFragment).commit();
          isSettingsFrag = true;
         // fragTrans.add(R.id.flFragment,settingsFragment).commit();
      }
     });
     /*
     //open the current date fragment and send key for date selected
     ...
     calendar.setOnDateChangeListener(new CalendarView.OnDateChangeListener() {
       ...
       public void onSelectedDayChange(@NonNull CalendarView calendarView, int year, int month, int day) {
        dateKey = year + "/" + month + "/" + day;
           ...
           //fragMan= getSupportFragmentManager();
           fragTrans = fragMan.beginTransaction();
           fragTrans.replace(R.id.flFragment,dateFragment).commit();
           isDateFrag = true;
         }
     });
     */
